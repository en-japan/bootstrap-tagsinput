...
!function (a) {
  ...
  b.prototype = {
    ...
    add: function (b, c) {
      ...
      if (!(d.options.maxTags && d.itemsArray.length >= d.options.maxTags || b !== !1 && !b)) {
        ...
        if (!b.toString().match(/^\s*$/)) {
          ...
          var h = d.options.itemValue(b), i = d.options.itemText(b), j = d.options.tagClass(b), k = a.grep(d.itemsArray, function (a) {return d.options.itemValue(a) === h})[0];
          if (!k || d.options.allowDuplicates) {
            ...
            var o = a(".tag", d.$container).filter(function () {return a(this).data("item") === k});
    ...
    remove: function (b, c) {
      var d = this;
      if (d.objectItems && (b = "object" == typeof b ? a.grep(d.itemsArray, function (a) {return d.options.itemValue(a) == d.options.itemValue(b)}) : a.grep(d.itemsArray, function (a) {return d.options.itemValue(a) == b}), b = b[b.length - 1]), b) {
        var e = a.Event("beforeItemRemove", {
          item: b,
          cancel: !1
        });
        if (d.$element.trigger(e), e.cancel)return;
        a(".tag", d.$container).filter(function () {return a(this).data("item") === b}).remove(), a("option", d.$element).filter(function () {return a(this).data("item") === b}).remove(), -1 !== a.inArray(b, d.itemsArray) && d.itemsArray.splice(a.inArray(b, d.itemsArray), 1)
      }
    ...
    pushVal: function () {
      var b = this, c = a.map(b.items(), function (a) {return b.options.itemValue(a).toString()});
      b.$element.val(c, !0).trigger("change")
    },
    build: function (b) {
      var e = this;
      if (e.options = a.extend({}, h, b), c(e.options, "itemValue"), c(e.options, "itemText"), d(e.options, "tagClass"), e.options.typeahead) {
      ...
      if (e.options.typeaheadjs) {
        ...
        e.$input.typeahead(null, j).on("typeahead:selected", a.proxy(function (a, b) {e.add(j.valueKey ? b[j.valueKey] : b), e.$input.typeahead("val", "")}, e))
      }
      e.$container.on("click", a.proxy(function (a) {e.$element.attr("disabled") || e.$input.removeAttr("disabled"), e.$input.focus()}, e)), e.options.addOnBlur && e.options.freeInput && e.$input.on("focusout", a.proxy(function (b) {
        if (0 === a(".typeahead, .twitter-typeahead", e.$container).length) {
          var c = e.$input.val();
          if (e.objectItems) {
            var d = a.Event("beforeFreeInputItemAdd", {item: c, cancel: !0});
            if (e.$element.trigger(d), d.cancel)return;
            c = d.item
          }
          e.add(c), e.$input.val("")
        }
        ...
        if (e.options.freeInput && (g(b, e.options.confirmKeys) || f)) {
          var h = f ? d.substr(0, e.options.maxChars) : d;
          if (e.objectItems) {
            var i = a.Event("beforeFreeInputItemAdd", {item: h, cancel: !0});
            if (e.$element.trigger(i), i.cancel)return;
            h = f ? i.item.substr(0, e.options.maxChars) : i.item
          }
          e.add(h), c.val(""), b.preventDefault()
        }
        {
          var j = c.val().length;
          Math.ceil(j / 5)
